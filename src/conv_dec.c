...
static int16_t *vdec_malloc(size_t n)
{
	...
	return (int16_t *)aligned_alloc(NEON_ALIGN, sizeof(int16_t) * n);
#else
	return (int16_t *) malloc(sizeof(int16_t) * n);
#endif
}
			   ...
			   unsigned state, uint8_t *out, int len)
{
	...
	// printf("Traceback iteration: %d, state: %u, path: %u\n", i, state, path);
		    ...
		    const int8_t *in, uint8_t *out)
{
	...
	rc = conv_decode(vdec, in, code->punc,
			 out, code->len, code->term);
